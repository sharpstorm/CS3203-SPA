...
class WithClause: public Clause {
 ...
 private:
  ...
  void crossProduct(const string* set1, const StmtList* set2,
                    pair_set<string, int>* output) {
    for (auto it = set2->begin(); it != set2->end(); it++) {
      output->insert({*set1, *it});
    }
  }

  void crossProduct(const StmtList* set1, const string* set2,
                    pair_set<int, string>* output) {
