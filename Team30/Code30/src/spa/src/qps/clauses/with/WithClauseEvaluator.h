...
using PKBAttributeQuerier =
    EntityValue(*)(const QueryExecutorAgent &agent, const StmtValue &stmt);

typedef StmtValueList StmtList;
...
class WithClauseEvaluator {
 ...
 private:
  ...
  PQLQueryResultPtr result;

  bool isEmptyResult() const;
  bool isIntegerIndependent(const PQLSynonymType &type) const;

  void evaluateOnIntAttributes();
  void evaluateOnStringAttributes();

  void evaluateOnStmtConst(const AttributedSynonym *constant,
  ...
  StmtValueSet queryForStatement(const PQLQuerySynonymProxy &syn) const;
  EntityValueSet queryForEntity(const PQLQuerySynonymProxy &syn) const;
