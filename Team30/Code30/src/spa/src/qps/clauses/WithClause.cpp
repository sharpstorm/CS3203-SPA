...
constexpr PKBAttributeQuerier CallsQuerier = [](PkbQueryHandler *pkbQueryHandler,
                                                const int &stmt) {
  return pkbQueryHandler->getCalledDeclaration(stmt);
};

constexpr PKBAttributeQuerier ReadQuerier = [](PkbQueryHandler *pkbQueryHandler,
                                               const int &stmt) {
  return pkbQueryHandler->getReadDeclarations(stmt);
};

constexpr PKBAttributeQuerier PrintQuerier = [](PkbQueryHandler *pkbQueryHandler,
                                                const int &stmt) {
...
bool WithClause::populateMap(PQLSynonymType type, SynToStmtMap *map,
  ...
  switch (type) {
    case PQL_SYN_TYPE_CALL:
      queryPkbForAttribute<CallsQuerier, PQL_SYN_TYPE_CALL>(pkbQueryHandler, map);
      return false;
    case PQL_SYN_TYPE_READ:
      queryPkbForAttribute<ReadQuerier, PQL_SYN_TYPE_READ>(pkbQueryHandler, map);
      return false;
    case PQL_SYN_TYPE_PRINT:
      queryPkbForAttribute<PrintQuerier, PQL_SYN_TYPE_PRINT>(pkbQueryHandler, map);
      return false;
    default:
      break;
  }
