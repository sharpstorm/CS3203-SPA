...
template <typename V>
class MapVectorTable : public IntTable<vector<V>> {
 public:
  explicit MapVectorTable(int size = 1) : IntTable<vector<V>>(size) {}
  ...
  const vector<V>& get(int key) const { return IntTable<vector<V>>::get(key); }

  void forEach(pkb::Function<int, vector<V>>& function) const {
    return IntTable<vector<V>>::forEach(function);
  }

  int size() const override { return IntTable<vector<V>>::size(); };
