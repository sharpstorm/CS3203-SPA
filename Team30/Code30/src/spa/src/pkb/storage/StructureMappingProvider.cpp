...
std::unordered_set<int> StructureMappingProvider::getStatementsOfType(StmtType stmtType) const {
  return statementStorage->getStatementsOfType(stmtType);
}

std::pair<int,int> StructureMappingProvider::getProcedureLines(std::string procedureName) const {
  return procedureStorage->getProcedure(procedureName);
}

std::string StructureMappingProvider::getProcedureForLine(int lineNumber) const {
