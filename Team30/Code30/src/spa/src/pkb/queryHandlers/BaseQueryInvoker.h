...
class BaseQueryInvoker {
 ...
 public:
  BaseQueryInvoker(
      IProvider<LeftValue, LeftType> *leftProvider,
      AbstractPredicateFactory<LeftValue,
                               LeftType> *leftPredicateFactory,
      AbstractPredicateFactory<RightValue,
                               RightType> *rightPredicateFactory)
  ...
  QueryResult<LeftValue, RightValue> query(
    ...
    if (arg1->isKnown()) {
      ...
      return store->query(
          leftPredicateFactory->getPredicate(arg1),
          arg2->getValue());
    } else {
      return store->query(
